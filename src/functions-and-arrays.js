...
function sumNumbers(numbers) {
  if(numbers.length > 0){
  let totalSum = 0;
  /* This method didn't work i'm not sure why
...
function averageNumbers(numbersAvg) {
  ...
  let totalNumber = sumNumbers(numbersAvg);
  let averageNumb = totalNumber / numbersAvg.length;
  return averageNumb;
}
else return null;
}
...
function avg(arr) {
  ...
  let averageOfArray = sumOfArray / arr.length;
...
if(wordsFind.length > 0){
  if(wordsFind.includes(wordsToSearch)){
    return true;
  }
  return false;
}
